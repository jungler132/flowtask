...
import { transferTask } from '../../api/tasksApi';
...
type Props = StackScreenProps<TasksStackParamList, 'TaskTransfer'>;

type ThemeRadii = (typeof import('../../theme'))['radii'];

function createTaskTransferStyles(colors: ThemeColors, radii: ThemeRadii, shadowCard: ViewStyle) {
  return StyleSheet.create({
    ...
    warn: { color: colors.text, fontSize: 15, lineHeight: 22, marginBottom: 16 },
    btnPick: {
      flexDirection: 'row',
      alignItems: 'center',
...
export default function TaskTransferScreen({ route, navigation }: Props) {
  const tabScrollBottom = useTabScrollBottomPadding();
  const { colors, radii, shadowCard } = useTheme();
  const styles = useMemo(
    () => createTaskTransferStyles(colors, radii, shadowCard),
  ...
  async function submit() {
    if (!allIds.length) {
      Alert.alert('Передача', 'Выберите новых исполнителей из списка или укажите их ID.');
      return;
    }
    setBusy(true);
    try {
      ...
      if (!resolved.length) {
        Alert.alert('Передача', 'Не удалось определить пользователей. Проверьте ID.');
        return;
      }
      await transferTask(taskId, resolved);
      navigation.goBack();
    } catch (e) {
      Alert.alert('Не удалось передать', formatApiErrorForUser(e));
    } finally {
      setBusy(false);
    }
  }
    ...
    <KeyboardAvoid>
      ...
      >
        <UserInvitePickerModal
          ...
          excludeIds={allIds}
          primaryLabel="Выбрать исполнителей"
        ...
        <View style={styles.card}>
          <Text style={styles.warn}>
            Текущие исполнители будут заменены выбранными. Убедитесь, что указаны все нужные люди.
          </Text>
          <Pressable
        ...
        <Pressable style={[styles.btn, busy && styles.disabled]} onPress={submit} disabled={busy}>
          <Text style={styles.btnText}>{busy ? '…' : 'Передать'}</Text>
        </Pressable>
      </ScrollView>
    </KeyboardAvoid>
  );
